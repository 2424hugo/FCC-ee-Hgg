...
@dataclass(frozen=True)
class TwoJetBatch:
    """A mini-batch with all first jets followed by all second jets.

    Combining the graphs allows the shared encoder to process ``2 * B`` jets
    in one call.  Besides reducing Python/kernel-launch overhead, this makes
    BatchNorm statistics identical for the two jet positions.
    """
    ...
    def pin_memory(self) -> "TwoJetBatch":
        """Support DataLoader's recursive pinned-memory fast path."""

        return TwoJetBatch(
            jets=self.jets.pin_memory(),
            high_level=self.high_level.pin_memory(),
            y=self.y.pin_memory(),
        )
...
def collate_two_jet_samples(samples: Sequence[TwoJetSample]) -> TwoJetBatch:
    ...
    graphs = [sample.jet1 for sample in samples]
    graphs.extend(sample.jet2 for sample in samples)
    return TwoJetBatch(
        jets=Batch.from_data_list(graphs),
...
class TwoJetParquetDataset(Dataset[TwoJetSample]):
    ...
    def __init__(
        ...
        *,
        ...
        self._type_values = np.asarray(
            sorted({int(value) for value in (type_vocabulary or ())}),
            dtype=np.int64,
        )
        self._columns = tuple(
            dict.fromkeys(
    ...
    def shard_index(self, index: int) -> int:
        """Return the containing shard for a global dataset index."""
    ...
    def _type_indices(self, raw_types: np.ndarray) -> np.ndarray:
        ...
        locations = np.searchsorted(self._type_values, raw_types)
        in_range = locations < len(self._type_values)
        known = np.zeros(raw_types.shape, dtype=bool)
        known[in_range] = self._type_values[locations[in_range]] == raw_types[in_range]
    ...
    def _jet_graph(self, event: ak.Record, jet_index: int) -> Data:
        ...
        if self.max_constituents is not None and len(energy) > self.max_constituents:
            selected = np.argpartition(energy, -self.max_constituents)[
    ...
    def high_level_statistics(
        self, indices: Sequence[int] | None = None
    ) -> tuple[Tensor, Tensor]:
        """Compute training statistics without constructing particle graphs.

        Only the small scalar/two-jet Parquet columns are read.  Selected
        global indices are grouped by shard, avoiding the former full dataset
        pass through :meth:`__getitem__`.
        """

        if indices is None:
            selected = np.arange(len(self), dtype=np.int64)
        else:
            selected = np.asarray(indices, dtype=np.int64)
        if selected.size < 2:
            raise ValueError("At least two training events are required")
        if np.any(selected < 0) or np.any(selected >= len(self)):
            raise IndexError("High-level-statistics index outside dataset")

        shard_ids = np.searchsorted(self._ends, selected, side="right")
        count = 0
        total = np.zeros(self.high_level_dim, dtype=np.float64)
        total_squared = np.zeros(self.high_level_dim, dtype=np.float64)
        columns = list(self.scalar_features + self.jet_features)
        for shard_index in np.unique(shard_ids):
            local = selected[shard_ids == shard_index].copy()
            if shard_index:
                local -= self._ends[shard_index - 1]
            records = ak.from_parquet(self.paths[shard_index], columns=columns)[local]
            arrays: list[np.ndarray] = []
            for name in self.scalar_features:
                arrays.append(np.asarray(ak.to_numpy(records[name]), dtype=np.float64))
            for name in self.jet_features:
                pairs = np.asarray(ak.to_numpy(records[name]), dtype=np.float64)
                if pairs.ndim != 2 or pairs.shape[1] != 2:
                    raise ValueError(f"Field {name!r} must have shape (events, 2)")
                arrays.extend((pairs.sum(axis=1), np.abs(pairs[:, 0] - pairs[:, 1])))
            values = np.column_stack(arrays)
            if not np.isfinite(values).all():
                raise ValueError("Non-finite values in high-level training features")
            count += len(values)
            total += values.sum(axis=0)
            total_squared += np.square(values).sum(axis=0)

        mean = total / count
        variance = (total_squared - count * np.square(mean)) / (count - 1)
        return (
            torch.from_numpy(mean.astype(np.float32)),
            torch.from_numpy(np.sqrt(np.maximum(variance, 0.0)).astype(np.float32)),
        )
...
class EdgeConvBlock(MessagePassing):
    ...
    def _safe_knn_graph(points: Tensor, batch: Tensor, k: int) -> Tensor:
        ...
        # PyG Batch stores each graph contiguously, so global node ids follow
        # directly from graph starts and dense slots.  This avoids a second
        # to_dense_batch scatter in every EdgeConv block.
        counts = mask.sum(dim=1)
        starts = counts.cumsum(dim=0) - counts
        node_ids = starts[:, None] + torch.arange(
        ...
        # Ranking by squared Euclidean distance avoids an unnecessary square
        # root.  Force FP32 here even under CUDA AMP: neighbour identities are
        # discrete and are more sensitive to reduced-precision ties than the
        # subsequent MLP operations.
        with torch.autocast(device_type=points.device.type, enabled=False):
            dense_float = dense_points.float()
            norms = dense_float.square().sum(dim=-1, keepdim=True)
            distances = norms + norms.transpose(1, 2)
            distances.add_(
                torch.bmm(dense_float, dense_float.transpose(1, 2)), alpha=-2.0
            )
...
class TwoJetParticleNet(nn.Module):
    ...
    def forward(self, batch: TwoJetBatch) -> Tensor:
        embeddings = self.jet_encoder(batch.jets)
        event_count = batch.y.numel()
        if embeddings.shape[0] != 2 * event_count:
            raise RuntimeError("Expected exactly two encoded jets per event")
        first = embeddings[:event_count]
        second = embeddings[event_count:]
