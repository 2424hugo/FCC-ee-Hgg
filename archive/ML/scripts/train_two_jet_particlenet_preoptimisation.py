...
Use already prepared train/validation/test files (recommended)::
    ...
    python train_two_jet_particlenet.py \
...
from two_jet_particlenet import (
    ...
    TwoJetParticleNet,
    estimate_high_level_statistics,
...
def run_epoch(
    ...
    with context:
        for batch in loader:
            ...
            correct += int(((logits >= 0) == (batch.y >= 0.5)).sum())
            count += n
...
@torch.no_grad()
def predict(
    model: nn.Module, loader: Iterable[TwoJetBatch], device: torch.device
...
def main() -> None:
    ...
    if has_predefined:
        ...
        type_vocabulary = (
            infer_particle_type_vocabulary(train_paths)
            if args.use_particle_type else ()
        )
        dataset_args = dict(
        ...
        type_vocabulary = (
            infer_particle_type_vocabulary(paths) if args.use_particle_type else ()
        )
    ...
    stats_loader = make_loader(
        Subset(train_dataset, train_indices), shuffle=False, **loader_args
    )
    mean, std = estimate_high_level_statistics(stats_loader)
    train_loader = make_loader(
        Subset(train_dataset, train_indices), shuffle=True, **loader_args
    )
    val_loader = make_loader(
        Subset(val_dataset, val_indices), shuffle=False, **loader_args
    )
    test_loader = make_loader(
        Subset(test_dataset, test_indices), shuffle=False, **loader_args
    )
